...
void VertexBuffer::Init(const std::vector<Vertex>& vertices, DX12Context context) {
	...
	HR(context.Device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
	...
	UpdateSubresources(context.CommandList.Get(), m_VBO.Get(), m_UploadHeap ,0, 0, 1, &vertexData);
